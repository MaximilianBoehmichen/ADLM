...
class DeepSDF(nn.Module):
    ...
    def __init__(self, num_shapes, latent_dim=64, hidden_dim=128, num_frequencies=8):
        ...
        self.pe = RFFPE_3D(num_frequencies=num_frequencies, sigma=10.0, seed=42)
...
if __name__ == "__main__":
    ...
    # add a L2 regularization term (weight decay) on the latent codes to prevent them from growing too large during training
    optimizer = torch.optim.Adam([
        {"params": model.net.parameters(), "lr": 1e-3},
        {"params": model.pe.parameters(), "lr": 1e-3},
        {"params": model.latent_codes.parameters(), "lr": 1e-3, "weight_decay": 1e-4}
    ])

    # 3. Training Loop (300 Epochs is now more than enough!)
    epochs = 300
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_coords, batch_targets, batch_indices in dataloader:
            ...
            # compute traditional reconstruction loss (MSE) between predicted SDF and target SDF
            recon_loss = F.mse_loss(preds, batch_targets)

            # Add a L2 regularization on the latent codes to prevent them from growing too large

            latent_reg = torch.mean(model.latent_codes(batch_indices) ** 2)
            loss = recon_loss + 1e-3 * latent_reg
    ...
    slice_coords = torch.stack([x_grid, y_grid, torch.full_like(x_grid, -0.2)], dim=-1).reshape(-1, 3).to(device)
