"""ResNet-style GNN classifier with relative positional message passing (v2).

From branch other-gnn. Key differences vs resgcn:
- Fourier-encoded *relative* positions (translation-invariant)
- Sum aggregation instead of mean
- Virtual node: global pool injected back to nodes after each block
- Mean+max pooling concatenated before head
- Dropout
"""
...
class ResGCNClassifierV2(nn.Module):
